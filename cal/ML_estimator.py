...
class MaxLikelihoodEstimators:
    ...
    def compute_rg_bis(self, w):
        ...
        for transition in self.rg.transitions:
            ...
            for enabled_transition in transition.from_state.outgoing:
                denom += weights[enabled_transition.name]
            probs[str(transition.from_state) +
                  "_" + str(transition.name) +
                  "_" + str(transition.to_state)] = weights[transition.name] / denom

        return probs

    def unfold_rg_with_list(self, eventlog_only):

        ev_language = pm4py.statistics.variants.log.get.get_language(self.ev.get_log())
        ...
        triples = {}
        triples[(init_state, tuple([]), 0)] = []

        self.traces = {}  # all traces that arrives to sink
        while not q.empty():
            state, trace, level = q.get()
            rg_path = triples[(state, tuple(trace), level)]
            if len(state.outgoing) > 0:
                for transition in state.outgoing:
                    ...
                    if len(new_trace) <= max_length:
                        if tuple(new_trace) in prefixes or not eventlog_only:
                            new_rg_path = rg_path + [str(transition.from_state) +
                                                     "_" + str(transition.name) +
                                                     "_" + str(transition.to_state)]
                            key = (new_state, tuple(new_trace), level + 1)
                            if key not in triples:
                                triples[key] = new_rg_path
                                q.put((new_state, new_trace, level + 1))
                            else:
                                triples[key] = triples[key] + new_rg_path
            else:
                tt = tuple(trace)
                if tt in ev_language or not eventlog_only:
                    if tt not in self.traces:
                        self.traces[tt] = rg_path
                    else:
                        self.traces[tt] = self.traces[tt] + rg_path

    def unfold_rg_with_subgraph(self, eventlog_only):
        ...
        while not q.empty():
            state, trace, level = q.get()
            graph = triples[(state, tuple(trace), level)]
            if len(state.outgoing) > 0:
                for transition in state.outgoing:
                    new_state = transition.to_state
                    new_trace = trace.copy()
                    ...
                    if len(new_trace) <= max_length:
                        if tuple(new_trace) in prefixes or not eventlog_only:
                            ...
                            if str(state) not in new_graph:
                                new_graph[str(state)] = {str(new_state): str(transition)}
                            else:
                                new_graph[str(state)].update({str(new_state): str(transition)})
                            key = (new_state, tuple(new_trace), level + 1)
                            if key not in triples:
                                triples[key] = new_graph
                                q.put((new_state, new_trace, level + 1))
                            else:
                                triples[key] = merge_dicts(triples[key], new_graph)
            else:
                tt = tuple(trace)
                if tt in ev_language or not eventlog_only:
                    if tt not in self.traces:
                        graph.update({"sink1": {}})
                        self.traces[tt] = graph
                    else:
                        self.traces[tt].update(graph)
    ...
    def estimate_emd(self, method, nw0, step, maxiter, minw, maxw, improvement_threshold, v=0):
        ...
        print("\t#############################################")
        loglh_after = self.ev.log_likelihood(ev_language, pn_language_after)
        print("\temd value with optimized values: " + str(emd_after))
        print("\tloglh value with optimized values: " + str(loglh_after))
        print("\tExecution time: " + str(time.time() - start_time) + " seconds")
        return {transition.name: w_after[i] for i, transition in enumerate(self.pn.get_transitions())}
