...
## Set Parameters
prismpath="~/Documents/prism/prism/bin/prism";
...
## Load Region Graph
load(sagepath+'.sage')
statelist=ptavalue['statelist']
alphabet=ptavalue['alphabet']
cardclocks=ptavalue['cardclocks']
...
def toCOSMOS(allones,isIsotropic,Dline):
    ...
    s+=printGRML_OnePlace('2500','Signal',1);
    s+=printGRML_transition(statelist,allones,isIsotropic,Dline);
    s+=printGRML_arc(statelist);
    for state in statelist:
        for trans in state['transition']:
            sign = action_to_signal(trans['action']);
            if sign < 0 :
                s+=printGRML_OneArcN('27', '2500', '16%d' %(trans['id']),-sign);
            else :
                s+=printGRML_OneArcN('27', '16%d' %(trans['id']),'2500',sign);
    s+='</model>\n';
    return(s);
