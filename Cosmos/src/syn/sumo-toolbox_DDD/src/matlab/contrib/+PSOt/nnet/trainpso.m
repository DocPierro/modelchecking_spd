...
%     trainParam.show  = NaN;           iterations to update display (NaN means never)
...
% Rev 1.0 - 8/31/05
%
% Slightly edited so it at least runs with Matlab 7.3 by Dirk Gorissen <dirk.gorissen@ua.ac.be>
% on 16 March 2007
...
if isstr(net)
  switch (net)
    case 'pnames',
      net = {...
             'show  = iterations to update display (NaN means never)';...
             ...
             'plotfcn  = plot function to use if show ~= NaN';...
    ...
    case 'pdefaults',
      trainParam.show  = NaN;    % iterations to update display (NaN means never)
      trainParam.maxit    = 1000;  % maximum iterations
      trainParam.popsz    = 25;    % population size
      trainParam.ac       = [2,2]; % acceleration constants (for type = 0)
      trainParam.inwt     = [0.9,0.4]; % inertia weights (for type = 0)
...
if (doValidation)
  VV.net = net;
  [vperf, El, Ac, N, Zb,Zi,Zl] = calcperf(net,X,VV.Pd,VV.Tl,VV.Ai,VV.Q,VV.TS);
...
[perf, El, Ac, N, Zb,Zi,Zl] = calcperf(net,pso_out(1:end-1),Pd,Tl,Ai,Q,TS);
