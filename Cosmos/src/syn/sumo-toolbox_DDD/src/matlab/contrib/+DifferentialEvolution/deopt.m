...
if isempty( FM_pop )
	...
	%----FM_pop is a matrix of size I_NPxI_D. It will be initialized------
	...
	assert( all(size(FM_pop) == [I_NP,I_D]) );
end
...
%-----Select which vectors are allowed to enter the new population------------
  for k=1:I_NP
      ...
      S_tempval = wrapObjective( FM_ui(k,:) );   % check cost of competitor
      I_nfeval  = I_nfeval + 1;
...
%----Output section----------------------------------------------------------
  ...
  I_iter = I_iter + 1;
	...
	% Algorithm needs objective to return a struct, easiest to just wrap it
	function out = wrapObjective( x )
		% TODO: implement constraints properly
		out.I_nc      = 0;% no constraints
		out.FVr_ca    = 0;% no constraint array
		out.I_no      = 1;% number of objectives (costs)
		out.FVr_oa(1) = fname( x, S_struct );
	end
end
